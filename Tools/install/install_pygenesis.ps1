...
.DESCRIPTION
    ...
    Con -InstallLlamaBinaries descarga llama-server (Vulkan) vía install_llama_binaries.ps1.
...
.PARAMETER InstallLlamaBinaries
    Tras copiar Tools, descargar binarios llama.cpp a pygenesis_inference/bin/.
.PARAMETER SkipLlamaBinaries
    No intentar descargar binarios llama.cpp al final (comportamiento por defecto).
.EXAMPLE
    .\Tools\install\install_pygenesis.ps1
    .\Tools\install\install_pygenesis.ps1 -RuntimeRoot "D:\PyGenesis" -InstallLlamaBinaries
#>
param(
    [string]$RuntimeRoot = "",
    [switch]$SkipModelDownload,
    [switch]$InstallLlamaBinaries,
    [switch]$SkipLlamaBinaries
)
...
$totalSteps = if ($InstallLlamaBinaries -and -not $SkipLlamaBinaries) { 6 } else { 5 }
...
$llamaScript = Join-Path $ScriptDir "install_llama_binaries.ps1"
if ($InstallLlamaBinaries -and -not $SkipLlamaBinaries) {
    Write-Host ""
    Write-Step 6 6 "Binarios llama.cpp (Vulkan)"
    if (-not (Test-Path $llamaScript)) {
        throw "No se encontró install_llama_binaries.ps1 en $ScriptDir"
    }
    & $llamaScript -RuntimeRoot $RuntimeRoot
    if ($LASTEXITCODE -ne 0) { throw "install_llama_binaries.ps1 falló ($LASTEXITCODE)" }
    Write-OK "Binarios instalados en $(Join-Path $inferenceDst 'bin')"
}
...
if (-not $InstallLlamaBinaries) {
    Write-Host "  1. Binarios llama.cpp: PyGenesis → Setup → «Descargar binarios Vulkan (automático)»"
    Write-Host "     o: .\Tools\install\install_llama_binaries.ps1 -RuntimeRoot `"$RuntimeRoot`""
} else {
    Write-Host "  1. Binarios llama.cpp: instalados (build $($manifest.llama_cpp.llama_build))"
}
