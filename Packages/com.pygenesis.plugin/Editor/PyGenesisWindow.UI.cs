...
public partial class PyGenesisWindow
{
    ...
    private void DrawSetupBanner()
    {
        if (!PyGenesisDependencyChecker.HasCriticalGaps())
        {
            return;
        }
        ...
        EditorGUILayout.HelpBox(
            PyGenesisDependencyChecker.GetSetupBannerSummary()
                + "\n\nCompleta la instalación en PyGenesis → Setup.",
            MessageType.Warning);

        if (GUILayout.Button("Abrir Setup…", GUILayout.Height(24)))
        {
            PyGenesisSetupWindow.ShowWindow();
        }
